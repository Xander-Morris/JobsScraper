...
export function AuthForms({ prompt }: { prompt?: string } = {}) {
  ...
  const error = mutation.error instanceof Error ? mutation.error.message : mutation.error ? 'Something went wrong' : null
  ...
  return (
    <div className="mx-auto mt-10 max-w-sm text-left">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <Button type="submit" disabled={mutation.isPending}>{mode === 'login' ? 'Log in' : 'Sign up'}</Button>
      </form>
    </div>
  )
}
