...
- `backend/` — Go API server, background scraper, resume parsing (via the Gemini API), and the digest email job
...
You'll need a Postgres database (Supabase works out of the box, or point it at any Postgres 14+ instance), a free [Gemini API key](https://aistudio.google.com/apikey), and Docker if you want to use the compose file.

1. Copy `backend/.env.example` to `backend/.env` and fill in `DATABASE_CONNECTION`, `SECRET_KEY`, and `GEMINI_API_KEY` at minimum
...
Order matters — Caddy requests a certificate on startup, and a failed ACME challenge is rate limited by Let's Encrypt:
